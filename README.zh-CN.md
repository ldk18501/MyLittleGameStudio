# MyLittleGameStudio 中文说明

MyLittleGameStudio，简称 MLGS，是一个面向 Codex 的 Unity + C# 独立游戏 AI 工作室。

它参考过 Claude Code Game Studios 的“多角色游戏工作室”思路，但现在已经明确收敛为：

- 只支持 Codex，不再考虑 Claude Code 兼容。
- 只面向 Unity + C#。
- 保留少量专职 agent，而不是几十个角色。
- 只暴露一个 `/mlgs` 入口；你用自然语言描述需求，工作室自动判断是启动、头脑风暴、规划、实现、修 bug、测试还是构建。
- 支持老板参与度：你可以当甩手掌柜，也可以当爱管细节的制作人。
- 保留 Dashboard，看员工们的实时工作状态。

## 快速开始

在 MyLittleGameStudio 仓库根目录安装本地 Codex 插件：

```powershell
codex plugin marketplace add .
```

然后在 Codex App 的插件页面安装 **my-little-game-studio**。如果当前 CLI 的 `codex plugin --help` 明确显示支持 `plugin add`，也可以执行 `codex plugin add my-little-game-studio@my-little-game-studio-local`；不同桌面版本的 CLI 子命令并不完全一致。

新开一个 Codex thread，然后输入：

```text
/mlgs 我想开始一个新的 Unity 游戏，低参与度
```

MLGS 会先判断项目状态，然后让你选择：

- A) 新游戏
- B) 接管已有 Unity 项目
- C) 继续当前项目
- D) 修复或切换项目指针

它还会让你选择参与度：

- A) 低参与度：你当甩手掌柜，MLGS 自主推进，只在重大关口问你
- B) 中参与度：默认模式，日常自动做，方向和架构大事先问
- C) 高参与度：你深度参与，MLGS 更常给 A/B/C/D 选项和草案

## 常用说法

你只需要记住一个入口：

```text
/mlgs 你的需求
```

常见说法：

| 你可以输入 | 用途 |
|---|---|
| `/mlgs 开始一个新的 Unity 游戏，低参与度` | 启动、建新项目、设置参与度 |
| `/mlgs 接管 D:\path\to\YourUnityGame` | 分析并接管已有 Unity 项目 |
| `/mlgs 看看当前状态，告诉我下一步` | 查看项目状态、员工动态、风险、下一步选项 |
| `/mlgs 帮我头脑风暴一个休闲割草游戏` | 想点子、整理参考、生成 pitch/支柱/概念包 |
| `/mlgs 把当前概念拆成开发计划` | 拆系统、Unity 技术方案、任务计划、原型策略 |
| `/mlgs 做一个最小原型验证核心手感` | 做原型，或记录跳过原型的风险 |
| `/mlgs 继续实现下一个任务` | 实现已批准的 Unity/C# 任务 |
| `/mlgs 生成并接入下一批正式美术` | 生成、处理、切图/导入、引用并在 Unity 内验收资产 |
| `/mlgs 把当前版本推进到 Vertical Slice` | 验证最终观感、真实接线、生产架构、性能和美术流水线 |
| `/mlgs 检查 Content Complete 成品度` | 拒绝占位内容、未接线功能、缺失引用和生产代码阻断项 |
| `/mlgs 检查图标、本地化和崩溃错误` | 验证 MLGS 负责的游戏内容发布子集 |
| `/mlgs 修一下这个编译错误` | 修 bug、编译问题、QA 失败 |
| `/mlgs 审查当前任务是否可以进入下一阶段` | 审查代码、设计、任务、阶段、构建或工作流 |
| `/mlgs 跑一轮验证` | 跑验证，或写手动 QA 计划 |
| `/mlgs 做构建预检` | Unity 构建或构建预检 |
| `/mlgs 打开 dashboard` | 刷新工作室看板 |
| `/mlgs 帮助` | 查看简短菜单 |

完整分组说明见 `workflow/command-index.md`。

## 员工角色

| 角色 | 负责 |
|---|---|
| Producer | 路由、状态、范围、任务分配 |
| Creative Director | 核心幻想、pitch、支柱、参考解读 |
| Game Designer | 系统、规则、数值、验收标准 |
| Unity Architect | Unity 架构、包、场景、构建风险 |
| Gameplay Developer | C# 玩法实现 |
| UI/UX Developer | HUD、运行时 UI、输入体验 |
| Technical Artist | Shader、VFX、正式美术生成、切图/导入、引用、视觉性能 |
| QA Lead | 验证、smoke check、构建就绪 |

## 状态文件

根目录的 `studio/state.json` 只是经过校验的模板，不是实时项目状态。

真实项目状态在：

```text
<UnityProject>/.mlgs/state.json
```

本地当前项目指针在：

```text
$CODEX_HOME/mlgs/current-project.json
```

旧 `.mlgs/state.yaml` 和 `studio/current-project.local.yaml` 仍可读取，只有明确执行迁移时才会转换。

它会被 git 忽略。

## 接管已有 Unity 项目

```text
/mlgs 接管 D:\path\to\YourUnityGame
```

MLGS 会检查：

- Unity version
- `Assets/`、`ProjectSettings/`、`Packages/manifest.json`
- 是否已有 `.mlgs/state.json` 或旧 `.mlgs/state.yaml`
- 是否有概念、设计、原型、任务计划、测试证据
- C# 和资产规模
- 当前最可能阶段
- 下一步应该做什么

确认接管后，它只会写：

```text
<UnityProject>/.mlgs/state.json
<UnityProject>/.mlgs/project.md
$CODEX_HOME/mlgs/current-project.json
```

不会默认改你的 Unity 生产文件。

## Dashboard

MLGS 每次执行路由任务都会记录：

```text
$CODEX_HOME/mlgs/logs/activity.jsonl
$CODEX_HOME/mlgs/runtime.json
$CODEX_HOME/mlgs/dashboard/studio-data.js
```

打开：

```text
dashboard/index.html
```

你可以看到员工状态、最近活动、当前项目、阶段、参与度和下一条建议指令。

## 工具脚本

```powershell
powershell -ExecutionPolicy Bypass -File tools/resolve-state.ps1 -AllowTemplate
powershell -ExecutionPolicy Bypass -File tools/detect-project-stage.ps1 -ProjectRoot E:/path/to/project
powershell -ExecutionPolicy Bypass -File tools/adopt-project.ps1 -ProjectRoot E:/path/to/project
powershell -ExecutionPolicy Bypass -File tools/adopt-project.ps1 -ProjectRoot E:/path/to/project -Apply
powershell -ExecutionPolicy Bypass -File tools/get-project-status.ps1 -AllowTemplate
powershell -ExecutionPolicy Bypass -File tools/init-project-state.ps1 -ProjectRoot E:/path/to/project -Name "My Game"
powershell -ExecutionPolicy Bypass -File tools/repair-pointer.ps1 -ProjectRoot E:/path/to/project
powershell -ExecutionPolicy Bypass -File tools/repair-pointer.ps1 -Clear
powershell -ExecutionPolicy Bypass -File tools/check-state.ps1
powershell -ExecutionPolicy Bypass -File tools/run-smoke-tests.ps1
powershell -ExecutionPolicy Bypass -File tools/preflight-task.ps1 -Command implement
powershell -ExecutionPolicy Bypass -File tools/validate-changes.ps1
powershell -ExecutionPolicy Bypass -File tools/migrate-state.ps1 -ProjectRoot E:/path/to/project
```

## 当前定位

MLGS 现在不是一个兼容多平台的模板仓库，而是 Codex 里的 Unity 小型 AI 工作室。

它的默认目标是：少问废话，保留专业分工，把你想做的游戏一步步推进到可玩的 Unity 项目。

