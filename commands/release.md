...
2. Before producing the first locked Release Candidate package, run `tools/preflight-task.ps1 -Command build -BuildReason release-candidate -View model`. Before a later final release rebuild, use `-BuildReason release`. Record each attempt with `tools/record-build-event.ps1`.
2. Icon:
