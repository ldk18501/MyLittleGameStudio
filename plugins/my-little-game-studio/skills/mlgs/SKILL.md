...
2. `<MyLittleGameStudio>/studio/state.yaml`
3. `<MyLittleGameStudio>/studio/config.md`
...
Resolve the target Unity project from `studio/state.yaml`.
...
| `mlgs build`, `打包`, `构建APK` | `commands/build.md` |
...
- Use high automation for planning and medium automation for production unless `state.yaml` says otherwise.
- Ask only for high-risk, destructive, architecture-changing, package-changing, or genuinely ambiguous decisions.
- Keep `studio/state.yaml` as the only root state source.
