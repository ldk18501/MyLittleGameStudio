...
- 九宫格 UI 遵循 `rules/nine-slice.md`：使用方只能改变配方声明的轴；需要双轴缩放的气泡、页签等把中段箭头/尾巴拆成独立子 `Image`。

## Effect-image component contract

- Before formal UI art generation, collaborate with the Art Director to audit every visible component in the approved target image.
- Map every interactive control to at least one audited visual component and record normal, hover, pressed, disabled, selected, checked, loading, or error states as applicable.
- Prefer shared `reuseKey` families for visually identical controls. Keep labels as runtime text unless the Art Director explicitly approves baked text.
