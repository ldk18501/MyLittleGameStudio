...
- 仅对骨骼/分件/蒙皮或含骨骼的混合角色维护 `design/art/characters/<character-id>.animation.json`；纯帧动画不创建该合同。
- 从作者图层、人工分层或确定性蒙版导出正式部件，只在合同蒙版内补全隐藏区域；维护唯一 `visibleRegionId`、Pivot、Socket、接缝覆盖和方向专用 Sorting。
...
- AI 爆炸拆件图只能作为草稿，不能作为骨骼角色正式切图源。进入 Unity 前必须重建标准姿势并验证部件所有权、上下肢旋转范围和接缝。
...
- 骨骼角色的 Prefab 从合同确定性构建；连续弯曲评估 Sprite Skin/网格权重，刚性装甲、武器和动作替换图保持显式独立层。禁止按包围盒中心猜 Pivot，或用 Transform 手调掩盖错误源图。
