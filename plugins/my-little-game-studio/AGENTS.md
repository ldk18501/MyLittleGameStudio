...
- Low owner participation delegates routine creative and design decisions; it never implies a smaller or more casual game. Sparse ideas must be autonomously expanded before production.
- Classify product depth as `hyper-casual`, `light`, `standard`, or `deep`. Reference-led, explicitly non-light, and 10+ hour products default to at least `standard`; 30+ hour products default to `deep` unless the owner approves a narrower promise.
- For standard/deep, reference-led, or sparse low-participation concepts, research current web sources across direct, adjacent, and contrast games. Record URLs, observed facts, inferences, adapted patterns, and rejected patterns; never fabricate research or copy protected expression.
- Before production, approve `design/content-architecture.json`. It must cover moment/session/medium/long loops, interacting systems, content families and quantities, progression arcs, variety and repetition control, differentiation, endgame, release-scope mappings, and a quantified content budget that supports the promised minimum playtime.
- Prototype scope is a risk test, not the product definition. A visually polished reproduction of the prototype fails product-depth review.
...
- Before formal UI asset generation, every approved screen target must receive an Art Director component audit. Enumerate each visible reusable or procedural UI element, record its pixel reference rectangle, states, reuse family, and production decision, then link every generated component to a `screen-derived` `visualComponent` entry in `production/assets/asset-manifest.json` with component-specific style and prompt constraints.
- Every representative production screen/scene must exist in `design/art/visual-scene-contract.json`. Formal art work starts with a composition blockout and fixed Unity capture setup; whole-screen target match, anchors, depth layers, material/lighting language, detail density, diegetic integration, and renderer ownership must pass before asset approval.
- Before production code, inspect and approve `design/framework-adoption.json`. Existing asmdefs, composition root, module lifecycle, events, configuration, persistence, UI framework, tests, and asset loading are adopted or explicitly adapted; task-local replacement frameworks and Demo/Test implementations are rejected.
- Classify code production as `new-project/lightweight`, `small-existing/standard`, or `large-framework/deep` in `design/code/codebase-profile.json`; the owner or Unity Architect may override automatic classification with a reason.
- New projects may create a minimal foundation without legacy-style evidence. Small existing projects require module-neighborhood context and real exemplars. Large framework projects require dependency-graph evidence using CodeGraph, Roslyn, or documented manual structural review; CodeGraph itself is never the only accepted provider.
- Every production code task requires a fresh `production/context-packs/<task-id>.json`, an approved `production/change-plans/<task-id>.json`, and planned-vs-actual conformance. Existing code may be extended, adapted, replaced, or isolated behind a new module when the approved legacy tradeoff justifies it.
- Before Unity implementation, approve `design/presentation-architecture.json`. In 2D games, core gameplay uses SpriteRenderer/TilemapRenderer scene content unless the owner explicitly approves a pure-UI game. UGUI/UI Toolkit is for HUD, menus, overlays, dialogs, inventories, tooltips, and narrow approved exceptions; UI never owns authoritative gameplay rules.
...
- Work packages carry a verification policy. Default to task-boundary cadence: aggregate small edits, use focused checks only for acceptance-critical or risk-triggered changes, and run routine compile/acceptance/integration verification once per attempt. Broader regression is triggered by relevant input changes, shared contracts, scene/prefab wiring, persistence/configuration, a previous failure, build/phase gates, or an owner request.
- Regression breadth and package-build cadence are separate. A full regression never authorizes a Unity/platform package build.
- A new project may run one initial target-platform package validation after platform confirmation. Record the result in `.mlgs/build-policy.json` and reuse it.
- After that validation passes, ordinary code/content/UI/art/config changes, fixes, and Vertical Slice through Beta gates use compile/editor/PlayMode/platform-preflight evidence only. Do not invoke Unity batch builds, Gradle/Xcode, signing, installation, or device deployment.
- A later development package requires an explicit request in the owner's current message. Automatic packaging is allowed only in Release Candidate/Release.
...
- 仅当 2D 角色使用骨骼分件、Sprite Skin 或含骨骼的混合动画时，读取并执行 `rules/character-animation-art.md`。先锁定方向专用视角、头身比、姿势引导、Rig Master、骨骼、唯一部件所有权、Pivot/Socket、接缝与确定性 Prefab；AI 爆炸拆件图不得作为正式源。纯帧动画、静态 Sprite、UI、图标和场景资源不进入该合同。
...
- Approved visual targets use a structured style lock. Every production prompt must include the target image as a real reference, copy the lock exactly, repeat preserve/avoid invariants on each edit, and pass `tools/test-art-prompt.ps1`; free-form “same style” wording is insufficient.
- `gpt-image-2` cannot use 512×512 as a model canvas because the output must contain at least 655,360 pixels, and it does not support transparent backgrounds. Treat small icon/portrait sizes as local post-process outputs. Eligible same-style assets may use a 2–9 item registered sheet with explicit rectangles and `tools/split-art-sheet.ps1`; animations, nine-slice panels, text UI, unregistered grids, edge contact, or cross-cell contamination fail closed.
- Every formal asset links `production/assets/usage/<asset-id>.json` describing Unity target objects/components, size, PPU/pivot/border, tint/color preservation, material, sorting, states, and Game View evidence; import success without correct usage metadata is incomplete.
