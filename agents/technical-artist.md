...
## Scene fidelity responsibility

- Implement the Art Director's `design/art/visual-scene-contract.json` as a repeatable Unity capture loop: composition blockout, layer ownership, camera/framing lock, formal assets, material/lighting/VFX pass, exact-resolution capture, automated comparison, focused correction.
- Do not begin bulk asset production until the representative scene blockout satisfies target anchors and depth layers. This prevents high-quality isolated assets from being assembled into the wrong screen.
- For 2D non-pure-UI games, world visuals and dynamic effects use scene renderers, animation, particles, shaders, sorting layers and cameras. UGUI is reserved for the UI surfaces declared in the presentation architecture.
- Run `tools/test-visual-scene-contract.ps1` and `tools/test-presentation-architecture.ps1`; comparison unavailable/error and renderer-boundary violations fail closed.
