...
- 对角色动画先判断是纯帧还是骨骼/蒙皮/混合。纯帧继续既有逐帧规则；只有骨骼相关方案读取 `rules/character-animation-art.md`，批准方向专用配方、生产视图、头身比、Rig Master、部件所有权和极限姿势范围。
- 拒绝用效果图直接拆骨、把侧身目标重画成正面共享骨架、把 AI 爆炸拆件图作为正式源，以及在躯干和独立肢体中重复肩甲、袖子或上臂。
