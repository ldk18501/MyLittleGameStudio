...
If only the template exists, route through `/mlgs` to the internal `start` flow before project-level work, unless the current request contains enough idea or path context to create/adopt the project directly.
...
2. If `.mlgs/state.yaml` exists, repair the pointer and route through `/mlgs` to status.
3. If it is a Unity project without MLGS state, route through `/mlgs` to adopt.
4. If it is not Unity but has docs/prototype/code, route through `/mlgs` to adopt for gap analysis.
